// Import types only since we use window.electronAPI for IPC calls
import type { SummarizationOptions, OCROptions, OCRResult } from '../../../config/ai'

export interface AISummaryResponse {
  success: boolean
  summary?: string
  error?: string
}

export interface AIBatchSummaryResponse {
  success: boolean
  results?: Array<{
    id: string
    success: boolean
    summary?: string
    error?: string
  }>
  error?: string
}

export interface AIHealthResponse {
  success: boolean
  status?: string
  error?: string
}

class AIApi {
  async summarizeNote(
    noteId: string,
    content: string,
    title: string,
    options?: SummarizationOptions
  ): Promise<AISummaryResponse> {
    try {
      // noteId parameter is kept for compatibility but not needed for API call
      const response = await window.electronAPI.aiAPI.summarizeNote(content, title, options)
      return response
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to summarize note'
      }
    }
  }

  async batchSummarize(
    notes: Array<{ id: string; content: string; title: string }>,
    options?: SummarizationOptions
  ): Promise<AIBatchSummaryResponse> {
    try {
      const response = await window.electronAPI.aiAPI.batchSummarize(notes, options)
      return response
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to batch summarize'
      }
    }
  }

  async extractTextFromImage(
    imagePath: string,
    options?: OCROptions
  ): Promise<OCRResult> {
    try {
      const response = await window.electronAPI.aiAPI.extractTextFromImage(imagePath, options)
      return response
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to extract text from image'
      }
    }
  }

  async extractTextFromBuffer(
    imageBuffer: ArrayBuffer,
    mimeType: string,
    options?: OCROptions
  ): Promise<OCRResult> {
    try {
      // Convert ArrayBuffer to Buffer for IPC
      const buffer = Buffer.from(imageBuffer)
      const response = await window.electronAPI.aiAPI.extractTextFromBuffer(buffer, mimeType, options)
      return response
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to extract text from buffer'
      }
    }
  }

  async checkHealth(): Promise<AIHealthResponse> {
    try {
      const response = await window.electronAPI.aiAPI.checkHealth()
      return response
    } catch (error) {
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Health check failed'
      }
    }
  }
}

export const aiApi = new AIApi()